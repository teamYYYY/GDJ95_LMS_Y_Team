...
public interface UniversityNoticeMapper {
		...
		// 학교 공지사항 등록 및 수정 및 삭제 기능 사용시 검증기능 (접속한 사람이 혹시 관리자인가?)
		// 세션에서 userNo, userId를 가져와서 비교해야함 
		Integer insertUpdateRemoveUniversityNoticeValidate(Map<String, Object> userValidateParams);
		
		// 학교 공지사항 입력 시 입력정보 우선순위 셀렉박스리스트
		List<UniversityNoticeDTO> selectUniversityNoticePriorityList();

		// 학교 공지사항 등록, 수정, 삭제 전에 한번 검증 하기
		Integer insertUpdRemvUniversityNoticeNoValidate(Integer universityNoticeNo);
